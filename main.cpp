    ...
    while (true) {
        ...
        if (optiune == 0) {
            ...
            try {
                ...
                std::cout<<"DEBUG: Creare obiect Persoana\n";
                std::cout.flush();

                Persoana p(numePersoana, anulNasterii);

                std::cout<<"DEBUG: Obiect Persoana creat cu succes\n";
                std::cout.flush();
                ...
                std::cout<<"DEBUG: Date citite - rand: "<<rand<<", coloana: "<<coloana<<"\n";
                std::cout.flush();

                const Bilet* b = nullptr;

                if (tipBilet == 1) {
                    std::cout<<"DEBUG: Incepe vindeBiletStandard\n";
                    std::cout.flush();

                    b = manager.vindeBiletStandard(p, *eveniment, rand, coloana, std::cout);

                    std::cout<<"DEBUG: vindeBiletStandard terminat\n";
                    std::cout.flush();
                } else if (tipBilet == 2) {
                    std::cout<<"DEBUG: Incepe vindeBiletReducere\n";
                    std::cout.flush();

                    b = manager.vindeBiletReducere(p, *eveniment, rand, coloana, std::cout);

                    std::cout<<"DEBUG: vindeBiletReducere terminat\n";
                    std::cout.flush();
                } else if (tipBilet == 3) {
                    int pop, baut;
                    ...
                    std::cout<<"DEBUG: Incepe vindeBiletVip\n";
                    std::cout.flush();

                    b = manager.vindeBiletVip(p, *eveniment, rand, coloana, pop, baut, std::cout);

                    std::cout<<"DEBUG: vindeBiletVip terminat\n";
                    std::cout.flush();
                } else {
                    throw std::runtime_error("Tip bilet invalid\n");
                }
                ...
                if (b!=nullptr) {
                    std::cout<<"DEBUG: Afisare detalii bilet\n";
                    std::cout.flush();

                    std::cout<<"Bilet vandut: "<<b->descriere()<<" | pret final: "<<b->calculeazaPret()<<"\n";

                    std::cout<<"DEBUG: Detalii bilet afisat\n";
                    std::cout.flush();
                }
            }
